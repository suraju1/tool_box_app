...
class _WebTradeDetailsScreenState extends State<WebTradeDetailsScreen> {
  ...
  void initState() {
    ...
    if (widget.tradeId != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        context.read<TradeController>().fetchTradeHistoryDetails(widget.tradeId!);
      });
    }
  }
      ...
      body: Center(
        child: ConstrainedBox(
          ...
          child: RefreshIndicator(
            onRefresh: () async {
              if (widget.tradeId != null) {
                await context.read<TradeController>().fetchTradeHistoryDetails(widget.tradeId!);
              }
            },
  ...
  // Action Buttons
  Widget _buildCompleteButtonWeb(BuildContext context, TradeResponseModel response) {
    final authController = context.read<AuthController>();
    final currentUserId = authController.currentUser?.id;
    final isResponder = currentUserId == response.responderId;

    final canComplete = isResponder && (response.status == 'accepted' || response.status == 'meeting_set');
  ...
  Widget _buildChatButtonWeb(BuildContext context, TradeResponseModel response) {
    final authController = context.read<AuthController>();
    final currentUserId = authController.currentUser?.id;
    ...
    return ElevatedButton.icon(
      onPressed: () {
        final otherUserId = isOwner ? response.responderId.toString() : response.posterUserId.toString();
        final otherUserName = isOwner ? response.responderName : (response.posterName ?? 'User');
        final otherUserImage = isOwner ? response.responderImage : response.posterImage;
      ...
      icon: Icon(Icons.chat_bubble_outline, color: context.onPrimaryColor, size: 20),
      label: Text(
        'Chat with ${isOwner ? response.responderName : (response.posterName ?? 'User')}',
        style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: context.onPrimaryColor),
      ),
      style: ElevatedButton.styleFrom(
  ...
  Widget _buildCancelButtonWeb(BuildContext context, TradeResponseModel response) {
    final authController = context.read<AuthController>();
    final currentUserId = authController.currentUser?.id;
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              final success = await context.read<TradeController>().cancelTrade(tradeId);
              if (mounted) {
                ToastService.showSuccessToast(
                  context,
                  success ? 'Trade cancelled successfully' : 'Failed to cancel trade'
                );
              }
            },
  ...
  Widget _buildMainItemCard(TradeResponseModel response) {
    final itemName = response.postItemName ?? response.givingItemName ?? 'Trade Item';
    final images = response.postItemImages.isNotEmpty ? response.postItemImages : (response.givingItemImages ?? []);
        ...
        children: [
          imageUrl.isNotEmpty
              ...
              : Container(
                  ...
                  child: Icon(Icons.image, color: context.dividerColor, size: 40),
                ),
              ...
              children: [
                ...
                Row(
                  children: [
                    _buildBadge(
                      isGive ? 'Give' : 'Take',
                      context.isDarkMode ? Colors.blue.withOpacity(0.15) : const Color(0xFFE8F1FF),
                    ...
                    _buildBadge(
                      ...
                      response.status == 'completed'
                          ? (context.isDarkMode ? Colors.green.withOpacity(0.15) : const Color(0xFFE8F9EE))
                          : (context.isDarkMode ? Colors.orange.withOpacity(0.15) : const Color(0xFFFFF4E8)),
                      response.status == 'completed' ? const Color(0xFF27AE60) : const Color(0xFFF2994A),
                    ),
                  ],
                ),
              ],
  ...
  Widget _buildUserCard(TradeResponseModel response) {
    ...
    final partnerName = isPoster ? (response.responderName) : (response.posterName ?? 'Poster');
    final partnerImage = isPoster ? (response.responderImage ?? '') : (response.posterImage ?? '');
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              onPressed: () {
                int partnerId = isPoster ? response.responderId : response.posterUserId;
    ...
    return Consumer<TradeController>(
      builder: (context, tradeController, _) {
        return Container(
          ...
          padding: const EdgeInsets.all(16),
          decoration: BoxDecoration(
            color: context.isDarkMode ? Colors.white10 : const Color(0xFFF9FAFB),
            borderRadius: BorderRadius.circular(16),
            border: Border.all(color: context.dividerColor.withOpacity(0.5)),
          ),
          child: Column(
  ...
  Future<void> _submitUserMark(TradeResponseModel response, int partnerId, String mark) async {
    ...
    if (success) {
      ...
      ToastService.showSuccessToast(context, mark == 'like' ? 'Liked successfully' : 'Disliked successfully');
    } else {
      ToastService.showErrorToast(context, tc.errorMessage ?? 'Failed to submit mark');
    }
  }
        ...
        children: [
          Container(
            ...
            child: imageUrl.isNotEmpty && !isTicketTrade
                ? AppCachedImage(imageUrl: imageUrl, fit: BoxFit.cover, radius: 8)
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(exchangeLabel, style: TextStyle(fontSize: 16, color: context.subTextColor)),
                const SizedBox(height: 4),
                Text(
                  exchangeValue,
                  style: TextStyle(
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 16),
          _buildInfoRow(Icons.access_time_outlined, 'Status', response.status.toUpperCase()),
          if (response.meetingType != null) ...[
            const SizedBox(height: 16),
            _buildInfoRow(Icons.handshake_outlined, 'Meeting', response.meetingType!.replaceAll('_', ' ').toUpperCase()),
          ],
        ],
      ),
    );
  ...
  Widget _buildInfoRow(IconData icon, String label, String text) {
    return Row(
      children: [
        ...
        const SizedBox(width: 16),
        Expanded(
          child: Text(label, style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: context.subTextColor)),
        ),
        Text(text, style: TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: context.textColor)),
      ],
    );
  }
  ...
  Widget _buildNotesCard(TradeResponseModel response) {
    ...
    return Container(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Icon(Icons.description_outlined, color: context.subTextColor, size: 28),
          const SizedBox(width: 16),
          Expanded(
            child: Text(
              notes,
